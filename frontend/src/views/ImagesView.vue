...
const hasFilters = computed(() => formatFilter.value !== 'all' || albumFilter.value !== 'all')
...
<template>
  <section class="content-page images-page">
    <div class="page-heading management-heading">
      <div><span class="eyebrow">LIBRARY</span><h1>{{ activeAlbum ? activeAlbum.name : '图片管理' }}</h1><p>{{ activeAlbum ? '正在查看该相册中的图片；移出相册不会删除原图。' : '筛选、预览并管理已上传的图片。' }}</p></div>
    ...
    <div class="image-filter-bar">
      ...
      <label><span>每页显示</span><span class="select-wrap"><select v-model.number="pageSize" @change="changePageSize"><option :value="12">12 张</option><option :value="24">24 张</option><option :value="48">48 张</option><option :value="96">96 张</option></select></span></label>
      <button v-if="hasFilters" class="ghost-button compact filter-reset" type="button" @click="clearFilters">清除筛选</button>
    </div>

    <div class="library-toolbar">
      <label class="check-control"><input type="checkbox" :checked="allSelected" @change="toggleAll" /><span>全选本页</span></label>
      ...
      <button class="secondary-button compact" type="button" :disabled="!selected.size || !albums.length || albumWorking" @click="albumPickerOpen = true">加入相册</button>
      ...
      <button class="danger-button" type="button" :disabled="!selected.size || deleting" @click="deleteSelected"><svg viewBox="0 0 24 24"><path d="M4 7h16m-10 4v6m4-6v6M9 7l1-3h4l1 3m3 0-1 13H7L6 7"/></svg>{{ deleting ? '正在删除…' : '批量删除' }}</button>
    </div>
    ...
    <div v-else-if="!images.length" class="empty-library">
      ...
      <h2>{{ hasFilters ? '没有符合条件的图片' : '这里还没有图片' }}</h2><p>{{ hasFilters ? '尝试调整格式或相册筛选条件。' : '上传第一张图片，之后就可以在这里管理。' }}</p>
      <button v-if="hasFilters" class="secondary-button" type="button" @click="clearFilters">清除筛选</button><RouterLink v-else class="secondary-button" to="/upload">去上传</RouterLink>
    </div>
    ...
    <div v-else-if="viewMode === 'grid'" class="image-grid">
      <article v-for="image in images" :key="image.id" class="image-card" :class="{ selected: selected.has(image.id) }">
        <button class="card-select" type="button" :aria-label="selected.has(image.id) ? '取消选择' : '选择图片'" @click="toggle(image.id)"><span>{{ selected.has(image.id) ? '✓' : '' }}</span></button>
        ...
        <div class="image-meta"><div><strong :title="image.originalName">{{ image.originalName }}</strong><span>{{ formatBytes(image.size) }} · {{ formatDate(image.createdAt) }}</span></div><button class="copy-button" type="button" :title="copied === image.id ? '已复制' : '复制图片链接'" @click="copyURL(image)"><svg v-if="copied !== image.id" viewBox="0 0 24 24"><rect x="8" y="8" width="11" height="11" rx="2"/><path d="M16 8V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2"/></svg><span v-else>✓</span></button></div>
      </article>
    </div>

    <div v-else class="image-list" role="list">
      <div class="image-list-heading" aria-hidden="true"><span></span><span>名称</span><span>大小</span><span>上传时间</span><span></span></div>
      <article v-for="image in images" :key="image.id" class="image-list-row" :class="{ selected: selected.has(image.id) }" role="listitem">
        <label class="list-select-control" :aria-label="selected.has(image.id) ? '取消选择' : '选择图片'"><input type="checkbox" :checked="selected.has(image.id)" @change="toggle(image.id)" /></label>
        <div class="list-image-identity"><button class="list-thumbnail preview-trigger" type="button" :aria-label="`预览 ${image.originalName}`" @click="openPreview(image)"><img :src="image.url" :alt="image.originalName" loading="lazy" /></button><div><strong :title="image.originalName">{{ image.originalName }}</strong><span>{{ image.mimeType }}</span></div></div>
    ...
    <AlbumPickerModal v-if="albumPickerOpen" :albums="albums" title="把图片加入相册" :description="`已选择 ${selected.size} 张图片，可以同时加入多个相册。`" confirm-text="加入相册" :busy="albumWorking" @close="albumPickerOpen = false" @confirm="addSelectedToAlbums" />
